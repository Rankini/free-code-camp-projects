...
function checkCashRegister(price, cash, cid) {
  ...
  const cidTotal = cid.reduce((a, b) => a+(b[1]*100), 0) //sum of cid in cents
  const changeOwed = (cash-price).toFixed(2)*100
  let changeStillDue = (cash-price).toFixed(2)*100
  let change = cid
    .reverse()
    .map(([unit, unitTotal], i) => {
      unitTotal = unitTotal*100
      const unitValue = VALUE[i][1]
      let unitCount = Math.round(unitTotal/unitValue)
      let changeTotal = 0
      while(changeStillDue > 0 && changeStillDue >= unitValue && unitCount > 0) {
        unitCount--
        changeStillDue -= unitValue
        changeTotal += unitValue
      }
      return [unit, changeTotal/100]
    })
    .filter(e => e[1])

  return (changeStillDue > 0 )
    ? {status: "INSUFFICIENT_FUNDS", change: []}
    : (cidTotal == changeOwed)
      ? {status: "CLOSED", change: [...cid.reverse()]}
